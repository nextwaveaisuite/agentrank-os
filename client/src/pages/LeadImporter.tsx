      ...
      <div style={{ maxWidth: 700, margin: "0 auto" }}>
        ...
        {step === 1 && (
          <div style={card}>
            ...
            <div
              ...
              onDrop={e => { e.preventDefault(); const file = e.dataTransfer.files[0]; if (file) { const reader = new FileReader(); reader.onload = ev => { const parsed = parseCSV(ev.target?.result as string); setLeads(parsed); setMsg(`✅ Parsed ${parsed.length} leads`); }; reader.readAsText(file); } }}
            >
        ...
        {step === 2 && (
          <div style={card}>
            ...
            <div style={{ background: "#0F172A", borderRadius: 8, padding: "12px 16px", marginBottom: 16 }}>
              <p style={{ margin: 0, fontSize: 13, color: "#94A3B8" }}>Sam will write a personalised email for each lead using their first name and location. Resend will deliver them automatically.</p>
            </div>
        ...
        {step === 3 && (
          <div style={card}>
            ...
            <p style={{ fontSize: 14, color: "#64748B", marginBottom: 20 }}>Sam will write a personalised email for each lead and Resend will deliver them automatically. This may take a few minutes for large batches.</p>
            <div style={{ background: "#0F172A", borderRadius: 10, padding: "16px 20px", marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                ...
                <span style={{ fontSize: 13, color: "#10B981", fontWeight: 700 }}>{affiliateUrl.substring(0, 30)}...</span>
              </div>
            </div>
        ...
        {step === 4 && (
          <div>
            <div style={{ ...card, background: "#052e16", border: "1px solid #166534" }}>
              <p style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#86efac" }}>✅ Campaign launched successfully!</p>
              <p style={{ margin: "4px 0 0", fontSize: 14, color: "#166534" }}>Emails are being delivered. Check back for open and click stats.</p>
            </div>
            <button onClick={() => { setStep(1); setLeads([]); setCampaignId(null); setMsg(""); }} style={{ width: "100%", padding: "12px", background: "#3B82F6", color: "#fff", border: "none", borderRadius: 8, fontWeight: 700, fontSize: 15, cursor: "pointer", marginBottom: 20 }}>
              ...
              <div>
                ...
                {stats.map((s: any) => (
                  <div key={s.id} style={card}>
                    <p style={{ margin: "0 0 12px", fontWeight: 700, fontSize: 15, color: "#F8FAFC" }}>{s.name}</p>
