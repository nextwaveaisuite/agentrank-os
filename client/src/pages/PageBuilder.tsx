...
export default function PageBuilder() {
  ...
  function pickTemplate(t: typeof templates[0]) {
    setSelected(t); setHeadline(t.headline); setSub(t.sub); setFields(t.fields);
  }
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", padding: "2rem" }}>
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>Landing Page Builder</h1>
        </div>
            ...
            {step === 1 && (
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {templates.map(t => (
                    <div key={t.id} onClick={() => pickTemplate(t)} style={{ padding: "14px 18px", borderRadius: 10, border: `2px solid ${selected.id === t.id ? t.color : "#e2e8f0"}`, background: selected.id === t.id ? t.color + "11" : "#fff", cursor: "pointer", fontWeight: 600, color: selected.id === t.id ? t.color : "#1e293b" }}>{t.label}</div>
                  ))}
                </div>
